...
-- Wed Apr  3 14:26:20 2024
-- Model: Barberia-workbench    Version: 1.0

-- Modelo de la barberia
...
CREATE TABLE IF NOT EXISTS `barberia`.`ServicioAtencion` (
  ...
  `Cantidad` INT NULL,
  `Total` FLOAT NULL,
  `idServicion` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idAtencion`, `idServicion`),
  INDEX `idServicio_idx` (`idServicion` ASC) VISIBLE,
  CONSTRAINT `idServicio`
    FOREIGN KEY (`idServicion`)
...
CREATE TABLE IF NOT EXISTS `barberia`.`Factura` (
  ...
  PRIMARY KEY (`idFactura`, `idElemento`),
  INDEX `idCliente_idx` (`idCliente` ASC) VISIBLE,
  INDEX `idEmpleado_idx` (`idEmpleado` ASC) VISIBLE,
  INDEX `idElemento_idx` (`idElemento` ASC) VISIBLE,
  ...
  CONSTRAINT `fkEmpleadof`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fkAtencionS`
    FOREIGN KEY (`idElemento`)
    REFERENCES `barberia`.`ServicioAtencion` (`idAtencion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fkCompraC`
    FOREIGN KEY (`idElemento`)
